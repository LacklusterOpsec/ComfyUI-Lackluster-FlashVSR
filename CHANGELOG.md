...
- **Streaming Loop Inference & Denoise Execution**: Fixed critical indentation bug across `src/pipelines/flashvsr_tiny.py`, `src/pipelines/flashvsr_tiny_long.py`, and `src/pipelines/flashvsr_full.py` where `model_fn_wan_video`, latent denoising (`cur_latents = cur_latents - noise_pred_posi`), temporal chunk decoding (`TCDecoder.decode_video`), color correction, and `frames_total.append(cur_frames)` were incorrectly indented inside the `else:` branch of `cur_process_idx == 0`. In FlashVSR streaming architecture, iteration 0 denoises the initial 6 latent frames, primes KV caches (`pre_cache_k`, `pre_cache_v`), and decodes the first 21 video frames. Skipping iteration 0 left KV caches uninitialized, created temporal dimension mismatches in `TCDecoder.decode_video`, and caused `RuntimeError: FlashVSR streaming pipeline produced no output frames`. Moving inference and decoding to the main loop body ensures all iterations (both initial 6-latent chunk and subsequent 2-latent chunks) execute and decode correctly.
- **Post-Loop Validation Placement**: Moved post-loop cache clears (`LQ_proj_in.clear_cache()`, `self.TCDecoder.clean_mem()`, `offload_model()`), output validation check (`if not frames_total:`), and final tensor concatenation (`torch.cat`) strictly outside the `for cur_process_idx` loop body across all pipelines.
- **Multi-Chunk & Tiled Streaming Unit Tests**: Added unit tests in `test_mock.py` covering multi-chunk streaming execution for `FlashVSRFullPipeline`, `FlashVSRTinyPipeline`, and `FlashVSRTinyLongPipeline` with both minimum frames (`num_frames=33`) and longer video lengths (`num_frames=193`) in both untiled and tiled VAE modes (`tiled=True`).

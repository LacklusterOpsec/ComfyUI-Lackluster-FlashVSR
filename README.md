...
- **🐛 Streaming Loop Inference & Denoise Execution Fix** - Fixed critical indentation bug across `FlashVSRTinyPipeline`, `FlashVSRTinyLongPipeline`, and `FlashVSRFullPipeline` where model inference (`model_fn_wan_video`), latent denoising, streaming decode (`TCDecoder.decode_video`), color correction, and frame accumulation (`frames_total.append`) were incorrectly indented inside the `else:` branch of `cur_process_idx == 0`. In FlashVSR, iteration 0 denoises the first 6 latent frames, primes KV caches (`pre_cache_k`/`pre_cache_v`), and decodes the initial 21 video frames. Skipping iteration 0 left KV cache uninitialized, created temporal dimension mismatches in `decode_video`, and caused `RuntimeError: FlashVSR streaming pipeline produced no output frames`. Moving inference and decoding to the main loop body ensures all streaming iterations execute and decode properly.
- **🐛 Post-Loop Output Validation Placement** - Moved post-loop cache clears (`clear_cache`, `clean_mem`, `offload_model`), output validation (`if not frames_total:`), and final tensor concatenation (`torch.cat`) strictly outside the `for cur_process_idx` loop body.
- **🧪 Multi-Chunk & Tiled Streaming Unit Tests** - Added regression test suite verifying streaming execution across multiple iterations and frame lengths (`num_frames=33` and `num_frames=193`) in both untiled and tiled VAE modes (`tiled=True`) across all pipeline variants.
